...
[[https://melpa.org/#/popterm][file:https://melpa.org/packages/popterm-badge.svg]]

[[file:demo/Screencast_20260410_163941.mp4][demo]]

[[file:demo/Screencast_20260410_163941-optimized.gif][gif]]
...
- *Smart Multi-Instance Management:* Create multiple terminal instances with
  ~C-u~ prefix (auto-numbered "1", "2", "3"...), cycle between them with
  flicker-free in-place swapping, and select from a ~completing-read~ menu when
...
~popterm~ requires Emacs 29.1+ and the [[https://github.com/tumashu/posframe][posframe]] package.
...
| ~popterm-toggle~          | ~C-u C-`~       | Create a new terminal instance (prompts for name)                 |
...
|-------------+------------------+----------------------------------------------------------|
| ~C-<next>~  | ~popterm-next~   | Cycle forward through terminals (flicker-free in-place)  |
| ~C-<prior>~ | ~popterm-prev~   | Cycle backward through terminals (flicker-free in-place) |
| ~C-q~       | ~popterm-return~ | Hide and return to the originating source buffer         |

*** Named Instances & Multi-Instance Management
...
Use ~C-u~ (universal argument) with ~popterm-toggle~ to create a new terminal
instance. You'll be prompted for a name, with auto-generated numeric defaults
("1", "2", "3", etc.):

#+BEGIN_SRC emacs-lisp
;; Press C-u C-` to create a new terminal
;; Prompts: "New terminal name (default 2): "
#+END_SRC

When multiple terminals exist for the current scope:
- If the popup is *visible*, pressing ~C-`~ hides it
- If the popup is *hidden*, pressing ~C-`~ shows a ~completing-read~ menu to
  select which terminal to display
- If only one terminal exists, it opens directly without prompting

**** Cycling Between Terminals

Inside any terminal, use ~C-<next>~ and ~C-<prior>~ to cycle between all
instances in your current scope:

- When the popup is *visible*, cycling swaps the buffer in-place without
  hide/show flicker
- When the popup is *hidden*, cycling shows the next/previous terminal
- The echo area displays your position: ~"popterm [2/5]"~ (current/total)

The cycling order respects your ~popterm-scope~ setting (project, frame, etc.).
...
#+BEGIN_SRC emacs-lisp
;; Sync terminal to the source buffer's directory when needed.
;; With reliable directory tracking, Popterm skips cd when already there.
...
- *PGTK emacs builds have a child frame focus issue*
  ...
  please see [[https://github.com/CsBigDataHub/popterm.el/issues/1][#1]] and [[https://lists.gnu.org/archive/html/bug-gnu-emacs/2025-05/msg00972.html][Emacs bug#64625]]
...
- *Multi-Instance Cycling:* Numeric index generation, flicker-free buffer swapping,
  cycle position messages, C-u prefix creation, completion menu triggers
