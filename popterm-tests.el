  ...
  (let ((buffer (get-buffer-create "*popterm-hide-order*"))
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'popterm--cleanup-posframe-state)
                    ...
                    ((symbol-function 'popterm--schedule-parent-focus-restore)
                     (lambda (frame)
...
(ert-deftest popterm-test-restore-parent-window-context-selects-source-window ()
  "Verify source-window restoration re-selects the original parent window."
  (let ((popterm--source-window 'source)
        (selected nil))
    (cl-letf (((symbol-function 'frame-live-p) (lambda (_frame) t))
              ...
              ((symbol-function 'select-window)
               (lambda (window &optional norecord)
                 (setq selected (list window norecord)))))
      (popterm--restore-parent-window-context 'parent)
      (should (equal selected '(source norecord))))))

(ert-deftest popterm-test-parent-focus-retry-skips-visible-posframe ()
  "Verify the parent-focus retry does not steal focus after a re-open."
    ...
    (cl-letf (((symbol-function 'frame-live-p) (lambda (_frame) t))
              ((symbol-function 'run-with-timer)
               (lambda (_delay _repeat fn)
                 (push fn retry-callbacks)
                 'fake-timer))
              ((symbol-function 'popterm--posframe-visible-p) (lambda () t))
              ((symbol-function 'popterm--restore-parent-window-context)
               (lambda (frame)
                 (setq restored frame)))
              ((symbol-function 'select-frame-set-input-focus)
               (lambda (frame)
                 (setq refocused frame))))
      (popterm--schedule-parent-focus-restore 'parent)
      (should retry-callbacks)
      ;; All retry callbacks should skip when posframe is visible
      (dolist (cb retry-callbacks)
        (funcall cb))
      (should-not restored)
      (should-not refocused))))

(ert-deftest popterm-test-parent-focus-retry-refocuses-hidden-posframe ()
  "Verify the parent-focus retry restores focus after the child is hidden."
  (let ((window-system 'pgtk)
        (retry-callbacks nil)
        (refocused nil)
        (restored nil)
    ...
    (cl-letf (((symbol-function 'frame-live-p) (lambda (_frame) t))
              ((symbol-function 'run-with-timer)
               (lambda (_delay _repeat fn)
                 (push fn retry-callbacks)
                 'fake-timer))
              ((symbol-function 'popterm--posframe-visible-p) (lambda () nil))
              ((symbol-function 'popterm--restore-parent-window-context)
               (lambda (frame)
                 (setq restored frame)))
              ((symbol-function 'select-frame-set-input-focus)
              ...
              ((symbol-function 'popterm--pgtk-force-focus)
               (lambda (frame)
                 (setq gtk-focused frame))))
      (popterm--schedule-parent-focus-restore 'parent)
      (should retry-callbacks)
      ;; Each retry callback should restore focus when posframe is hidden
      (funcall (car retry-callbacks))
      (should (eq restored 'parent))
      (should (eq refocused 'parent))
      (should (eq gtk-focused 'parent)))))
